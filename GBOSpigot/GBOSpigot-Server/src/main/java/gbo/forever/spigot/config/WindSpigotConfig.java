package gbo.forever.spigot.config;
...
import gbo.forever.spigot.async.pathsearch.AsyncNavigation;
...
import gbo.forever.spigot.entity.EntityTickLimiter;
...
public class WindSpigotConfig {
	...
	public static void init(File configFile) {
		...
		try {
			GBOSpigot.LOGGER.info("Loading WindSpigot config from " + configFile.getName());
	...
	private static void tracking() {
		disableTracking = true; // GBO - this async entity tracking stuff is a bunch of shit, breaks all our plugins
	...
	private static void serverBrandName() {
		serverBrandName = getString("settings.brand-name", "GBOSpigot");
	}
