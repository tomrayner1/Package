...
import gbo.forever.spigot.exception.ExploitException;
	...
	protected void a(ChannelHandlerContext ctx, Packet packet) throws Exception {
		if (this.channel.isOpen()) {
			if (this.m instanceof PlayerConnection) {
				try {
					for (gbo.forever.spigot.protocol.PacketListener packetListener : GBOSpigot.getInstance()
							.getPacketListeners()) {
		...
		if (this.isConnected()) {
			...
			if (!shouldCheckPacket) {
		        ...
		        if (WindSpigotConfig.asyncKnockback && (packet instanceof PacketPlayOutEntityVelocity || packet instanceof PacketPlayOutPosition || packet instanceof PacketPlayInFlying.PacketPlayInPosition || packet instanceof PacketPlayInFlying)) {
		        	...
		        	GBOSpigot.getInstance().getKnockbackThread().addPacket(packet, this, null);
		            return;
		        }
			}
	        // WindSpigot end
