...
import gbo.forever.spigot.random.FastRandom;
...
import gbo.forever.spigot.tickloop.ReentrantIAsyncHandler;
import gbo.forever.spigot.tickloop.TasksPerTick;
import gbo.forever.spigot.world.WorldTickManager;
// WindSpigot end
...
public abstract class MinecraftServer extends ReentrantIAsyncHandler<TasksPerTick> implements ICommandListener, IAsyncTaskHandler, IMojangStatistics {
	...
	// GBO - instance
	protected gbo.forever.spigot.GBOSpigot gboSpigot;
		...
		try {
			...
			if (this.init()) {
				...
				if (WindSpigotConfig.threadAffinity) {
					...
					gboSpigot.debug(AffinityLock.dumpLocks());
...
//        this.methodProfiler.b();
	...
	// WindSpigot - instance
	public GBOSpigot getWindSpigot() {
		return this.gboSpigot;
	}
