package uk.rayware.nitrolib.lunarclient;

import org.jetbrains.annotations.NotNull;
import uk.rayware.nitrolib.NitroLib;
import uk.rayware.nitrolib.lunarclient.events.*;
import uk.rayware.nitrolib.lunarclient.listener.LunarClientLoginListener;
...
import uk.rayware.nitrolib.lunarclient.object.*;
import uk.rayware.nitrolib.lunarclient.util.LunarClientDefaultNetHandler;
...
import uk.rayware.nitrolib.lunarclient.waypoint.WaypointMGR;
...
public class LunarClientAPI {
	...
	@Getter
	private final WaypointMGR waypointMGR = new WaypointMGR(this);
	
	public void log(String message) {
		Bukkit.getLogger().info("[NitroLib/LC-API] " + message);
	}
	
	/**
	 * Allows printing of an array, laterally, supply strings WITHOUT spaces as this function adds one between EVERY element.
	 * @param sectors The {@link String...} array to print.
	 */
	public void log(String @NotNull ... sectors) {
		StringBuilder FULL = new StringBuilder();
		for (String sector : sectors) {
			FULL.append(sector).append(" ");
		}
		Bukkit.getLogger().info("[NitroLib/LC-API] " + FULL.toString());
	}
	...
	public LunarClientAPI() {
		...
		NitroLib.getInstance().getServer().getPluginManager().registerEvents(new LunarClientLoginListener(this), NitroLib.getInstance());
		
		log("LunarClientAPI has initialized.");
	}
	
	/**
	 * Registers the bukkit plugin channel based on configuration of allowed players
	 * Will stop if packet is null!
	 */
	private void registerPluginChannel() {
		Messenger messenger = getServer().getMessenger();
		messenger.registerOutgoingPluginChannel(NitroLib.getInstance(), LunarClientAPI.MESSAGE_CHANNEL);
		messenger.registerIncomingPluginChannel(NitroLib.getInstance(), LunarClientAPI.MESSAGE_CHANNEL, (channel, player, bytes) -> {
	...
	public Set<Player> getPlayersRunningLunarClient() {
		return playersRunningLunarClient.stream().map(Bukkit::getPlayer).collect(Collectors.toUnmodifiableSet());
	}
	...
	/***/
	public void broadcastWaypoint(LCWaypoint waypoint) {
		for (UUID player: playersRunningLunarClient) {
			Player ply = Bukkit.getPlayer(player);
			if (ply != null) {
				sendWaypoint(ply, waypoint);
			}
		}
	}
	...
	public void sendPacket(Player player, LCPacket packet) {
		...
		if (isRunningLunarClient(playerId)) {
			player.sendPluginMessage(NitroLib.getInstance(), MESSAGE_CHANNEL, LCPacket.getPacketData(packet));
	...
	/**
	 * Broadcasts a packet to all players. Does not differentiate between players
	 *
	 * @param packet The {@link LCPacket} to send to the player/s.
	 * */
	public void broadcastPacket(LCPacket packet) {
