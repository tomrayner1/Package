...
import gbo.forever.spigot.events.PlayerIllegalBehaviourEvent;
...
public class PlayerConnection implements PacketListenerPlayIn, IUpdatePlayerListBox {
		...
		if (this.b(packetplayinflying)) {
			...
			if (!this.player.viewingCredits) {
				...
				if ((packetplayinflying.hasPos) && ((delta > 0.0D) && (this.checkMovement && !this.player.dead))) {
					for (MovementListener movementListener : GBOSpigot.getInstance().getMovementListeners()) {
						try {
				...
				if ((delta > 1f / 256 || deltaAngle > 10f) && (this.checkMovement && !this.player.dead)) {
					...
					if (WindSpigotConfig.firePlayerMoveEvent && PlayerMoveEvent.getHandlerList().getRegisteredListeners().length != 0) { // WindSpigot - only handle player move event when there are listeners // Spigot - don't skip any move events
						...
						if (event.isCancelled()) {
		                    GBOSpigot.getInstance().getLagCompensator().registerMovement(player, to); // Nacho
                    ...
                    GBOSpigot.getInstance().getLagCompensator().registerMovement(player, to); // Nacho - register movement
				}
	...
	public void sendPacket(final Packet packet) {
		...
		try {
			for (PacketListener packetListener : GBOSpigot.getInstance().getPacketListeners()) {
				try {
